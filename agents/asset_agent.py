    ...
    if not bank_files:
        ...
        for a in checking + trust:
            result.findings.append(make_finding(
                ...
                f"${a.amount:,.2f}", "", None, force_missing=True))
        result.files_to_reupload.append({
            "file_type": "Bank Statement",
            "reason": "Không tìm thấy file Bank Statement nào được upload",
            "suggested_name": "bankstatement_YYYY-MM-DD_<account>.pdf",
        })
        ...
        for a in checking + trust:
            ...
            if found:
                ...
                stmt_date = found.get("statement_date", "")
                holder_pdf = found.get("holder_name", "")
                holder_ok = a.holder.split()[-1].lower() in holder_pdf.lower() if holder_pdf else None
                ...
                result.findings.append(make_finding(
                    ...
                    "Required", stmt_date, bool(stmt_date)))
                result.findings.append(make_finding(
                    f"{a.holder} [{a.asset_type}] ...{t4} — Holder",
                    a.holder, holder_pdf, holder_ok))
                ...
                result.findings.append(make_finding(
                    f"{a.holder} [{a.asset_type}] ...{t4} — Statement Date",
                    "Required", "", None, force_missing=True))
                result.files_to_reupload.append({
                    "file_type": "Bank Statement",
                    "reason": f"Không tìm thấy account ...{t4} ({a.holder} {a.asset_type}) trong các statement đã upload",
                    "suggested_name": f"bankstatement_<date>_{t4}.pdf",
                })
                result.mismatches.append(
                    f"Không tìm thấy account ...{t4} ({a.holder} {a.asset_type}) trong bank statements")
                console.print(f"  [yellow]⚠ {a.holder} ...{t4}: không khớp → MISSING[/yellow]")
    ...
    if not brok_files:
        ...
        for a in stocks:
            result.findings.append(make_finding(
                ...
                f"${a.amount:,.2f}", "", None, force_missing=True))
        result.files_to_reupload.append({
            "file_type": "Brokerage Statement",
            "reason": "Không tìm thấy file Brokerage Statement nào được upload",
            "suggested_name": "brokeragestatement_YYYY-MM-DD_<account>.pdf",
        })
        ...
        for a in stocks:
            ...
            if found:
                ...
                result.files_to_reupload.append({
                    "file_type": "Brokerage Statement",
                    "reason": f"Không tìm thấy account {a.account_id} ({a.holder}) trong các brokerage statement đã upload",
                    "suggested_name": f"brokeragestatement_<date>_{t4}.pdf",
                })
                result.mismatches.append(
                    f"Không tìm thấy account {a.account_id} ({a.holder}) trong brokerage statements")
                console.print(f"  [yellow]⚠ {a.holder} {a.account_id}: không khớp → MISSING[/yellow]")
