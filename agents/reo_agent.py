...
def reo_agent_node(state: GraphState) -> GraphState:
    ...
    if not ins_files:
        ...
        result.files_to_reupload.append({
            "file_type": "Insurance Document",
            "reason": "Không tìm thấy file Insurance (bảo hiểm tài sản) nào được upload",
            "suggested_name": "washingtoninsurance.pdf hoặc clarksoninsurnace.pdf",
        })
        for r in xml_props:
    ...
    if not reo_files:
        ...
        result.files_to_reupload.append({
            "file_type": "REO Documentation",
            "reason": "Không tìm thấy file REO Documentation nào được upload",
            "suggested_name": "REO Documentation.pdf",
        })
        for r in xml_props:
            result.findings.append(make_finding(
                f"REO — Estimated Value: {r.address[:30]}",
                f"${r.estimated_value:,.0f}", "", None, force_missing=True))
            result.findings.append(make_finding(
                f"REO — Usage Type: {r.address[:30]}",
                r.current_usage, "", None, force_missing=True))
        ...
        try:
            ...
            for bx in xml_props:
                ...
                if found:
                    ...
                    result.findings.append(make_finding(
                        f"REO — Usage Type: {bx.address[:30]}",
                        bx.current_usage, "", None, force_missing=True))
                    result.findings.append(make_finding(
                        f"REO — Rental Income: {bx.address[:30]}",
                        f"${bx.rental_income_gross:,.0f}/mo", "", None, force_missing=True))
                    result.mismatches.append(
