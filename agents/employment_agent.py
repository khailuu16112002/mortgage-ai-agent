...
def employment_agent_node(state: GraphState) -> GraphState:
    ...
    for emp in baseline.employments:
        ...
        if emp.self_employed:
            ...
            if not biz_files:
                ...
                result.files_to_reupload.append({
                    "file_type": "Business Tax Return",
                    "reason": f"Thiếu Business Tax Return cho {emp.employer_name} ({emp.borrower_name})",
                    "suggested_name": "Business_Tax_Return_<year>.pdf hoặc greenwave_us_<year>.pdf",
                })
                result.findings.append(make_finding(
                    f"{emp.borrower_name} — Business Tax Return",
                    f"{emp.employer_name}", "", None, force_missing=True))
                result.findings.append(make_finding(
                    f"{emp.borrower_name} — Monthly Income (Self-Employed)",
                    f"${emp.monthly_income_total:,.0f}/mo", "", None, force_missing=True))
        ...
        # ── W-2 employee (Rebecca) ─────────────────────────────────────────────
            ...
            if not w2_files:
                ...
                result.files_to_reupload.append({
                    "file_type": "W-2 Form",
                    "reason": f"Thiếu W-2 cho {emp.borrower_name} từ {emp.employer_name}",
                    "suggested_name": "W2.pdf hoặc W2(1).pdf",
                })
                result.findings.append(make_finding(
                    f"{emp.borrower_name} — W-2 Employer",
                    emp.employer_name, "", None, force_missing=True))
                result.findings.append(make_finding(
            ...
            if not pay_files:
                ...
                result.files_to_reupload.append({
                    "file_type": "Pay Stub",
                    "reason": f"Thiếu Pay Stub gần nhất cho {emp.borrower_name}",
                    "suggested_name": "payslip_<date>_<name>.pdf",
                })
                result.findings.append(make_finding(
                    f"{emp.borrower_name} — Paystub",
                    "Required (30 ngày gần nhất)", "", None, force_missing=True))
    ...
    if not tax_files:
        ...
        result.files_to_reupload.append({
            "file_type": "Personal Tax Return (1040)",
            "reason": "Thiếu Personal Tax Return 1040 — cần cả 2023 và 2024",
            "suggested_name": "2024taxesnopasswordfederal.pdf, 20231040patrickbeckydurst.pdf",
        })
        result.findings.append(make_finding(
            "Personal Tax Returns (1040)", "2 năm (2023, 2024)", "", None, force_missing=True))
