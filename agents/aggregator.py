...
- Total counts ở cuối
- Gom danh sách FILE CẦN UPLOAD LẠI cho user
"""
...
def aggregator_node(state: GraphState) -> GraphState:
    ...
    # Gom tất cả files cần upload lại, loại trùng theo (file_type + suggested_name)
    seen_files = set()
    all_reupload = []
    for r in results:
        for item in r.files_to_reupload:
            key = (item.get("file_type",""), item.get("suggested_name",""))
            if key not in seen_files:
                seen_files.add(key)
                all_reupload.append(item)
    ...
    for r in results:
        ...
        n_bad = sum(1 for f in r.findings if not f.matched and f.matched is not None and not f.is_missing)
    ...
    # ── 📋 FILES CẦN UPLOAD LẠI ───────────────────────────────────────────────
    if all_reupload:
        console.print("\n[bold yellow on red] 📋 FILES CẦN UPLOAD LẠI [/bold yellow on red]")
        reupload_table = Table(box=box.SIMPLE, show_header=True, header_style="bold yellow")
        reupload_table.add_column("#",          width=4,  justify="right")
        reupload_table.add_column("Loại File",  width=28, style="yellow")
        reupload_table.add_column("Lý Do",      width=55, style="white")
        reupload_table.add_column("Tên File Đề Xuất", width=40, style="cyan")
        for i, item in enumerate(all_reupload, 1):
            reupload_table.add_row(
                str(i),
                item.get("file_type", "—"),
                item.get("reason", "—"),
                item.get("suggested_name", "—"),
            )
        console.print(reupload_table)

    # ── Missing documents (raw list) ──────────────────────────────────────────
    if all_missing:
    ...
    for r in results:
        ...
        # Group: FAIL first, then MISSING, then PASS
        fail_fields    = [f for f in r.findings if not f.matched and f.matched is not None and not f.is_missing]
        ...
        if fail_fields:
            console.print("  [bold red]  ── FAIL (dữ liệu sai) ──[/bold red]")
        ...
        if missing_fields:
            console.print("  [bold yellow]  ── MISSING (không đọc được / thiếu file) ──[/bold yellow]")
            for f in missing_fields:
                console.print(
                    f"    [yellow]⚠[/yellow] {f.field_name:<52} "
                    f"[dim]XML:[/dim] {str(f.xml_value):<30} "
                    f"[dim]→ Cần upload lại file[/dim]"
                )
    ...
    total_bad = sum(sum(1 for f in r.findings if not f.matched and f.matched is not None and not f.is_missing) for r in results)
    ...
    if all_pass:
        ...
        parts = []
        if all_mismatches:
            parts.append(f"[red]{len(all_mismatches)} dữ liệu sai[/red]")
        if all_reupload:
            parts.append(f"[yellow]{len(all_reupload)} file cần upload lại[/yellow]")
        console.print(
            f"[bold]⚠  KẾT LUẬN: CÓ VẤN ĐỀ[/bold] — " + " | ".join(parts)
        )
    console.rule()

    # Cập nhật state
    state.all_pass     = all_pass
    state.missing_files_summary = all_reupload
    state.final_report = (
        f"{'PASS' if all_pass else 'FAIL'} | "
        f"Missing: {len(all_missing)} docs | "
        f"Mismatches: {len(all_mismatches)} | "
        f"Files to reupload: {len(all_reupload)}"
    )
    return state
